<template>
	<div >
		<!-- <div class="container1 wl-container"><h1>1111</h1></div> -->
<div class="bigbox">
			<div class="left">
			<div class="wl-col-3  box" v-for="(article, index) in articals.slice(1,20)" :key="index" >
				
				<p class="tupian" ><img :src="article.cover" class="cover"  /></p>
			
				
				<p class="wl-subtitle ">{{ article.title }}</p>
	...
	created() {
		this.axios.get('http://localhost:8080/article').then(res => {
			...
			for (var i = 0; i < this.articals.length; i++) {
				this.articals[i].cover = this.getImage(this.articals[i].cover);
			}
		});
	},
	methods: {
